...
export async function POST(req: NextRequest) {
  try {
    const body = (await req.json()) as Partial<PetsBookingPayload>;
    const {
      departureCountry,
      departureCity,
      arrivalCountry,
    ...
    if (missing.length > 0) {
      await sendErrorReport({
        endpoint: "/api/send-booking-pets",
    ...
    const subject = `Pet transfer request: ${departureCity} → ${arrivalCity}${
      date ? ` (${date})` : ""
    }`;
    const text = `New PET TRANSFER request\n\nFrom: ${departureCountry}, ${departureCity}\nTo: ${arrivalCountry}, ${arrivalCity}${
      date ? `\nDate: ${date}` : ""
